...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["video"])) {
    ...
    if (move_uploaded_file($_FILES["video"]["tmp_name"], $targetFilePath)) {
        ...
        $stmt = $conn->prepare("INSERT INTO sports (title, file_path, uploaded_by) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $videoName, $targetFilePath, $user_email);
        if ($stmt->execute()) {
            echo "<script>alert('Video uploaded successfully!'); window.location.href='sports.php';</script>";
        } else {
            echo "<script>alert('Database error!');</script>";
        }
        $stmt->close();
    } else {
        echo "<script>alert('File upload failed!'); window.location.href='sports.php';</script>";
    }
}
$conn->close();
?>
